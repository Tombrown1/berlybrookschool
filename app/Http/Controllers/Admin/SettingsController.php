...
class SettingsController extends Controller
{
   ...
   public function logoBanner(Request $request)
      {
         $this->validate($request, [
            'banner' => 'required|image|mimes:jpeg,jpg,png,gif,svg,pdf|max:2048'
         ]);

         if($request->file('banner'))
         {
            $file = $request->file('banner');
            $banner = Storage::disk('public')->putFile('banner', $file);
         }

         $bannerUpload = Setting::find(1);
         $bannerUpload->banner = $banner;
         $bannerUpload->save();
